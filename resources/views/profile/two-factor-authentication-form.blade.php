<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Añade seguridad adicional a tu cuenta usando una autenticación de dos factores.') }}
    </x-slot>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                ...
                @endif
            @else
                {{ __('Todavía no has habilitado la autenticación de dos factores.') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Cuando la autenticación de dos factores está habilitada, se le solicitará un token aleatorio seguro durante la autenticación. Puede recuperar este token desde la aplicación Google Authenticator, Aegis o Authy de su teléfono.') }}
            </p>
        </div>
